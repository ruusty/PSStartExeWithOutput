...
Describe "Start-ExeWithOutput" {
  ...
  Context -Name 'Input'{
    It "Should fail for non-Existant Exe" -Pending {
      { Start-ExeWithOutput "not-found.exe" } | Should  throw
    }

  It "Should fail with no args for Ping.exe" {
      { Start-ExeWithOutput "ping.exe" } | Should  throw
    }

    It "Should succeed with single arg for Ping.exe" {
      {Start-ExeWithOutput "ping.exe" "8.8.8.8" @Verbose} | Should not throw
    }

    It "Should succeed with array of args for Ping.exe" {
      {Start-ExeWithOutput -filepath "ping.exe" @("8.8.8.8", "-n", "5") @Verbose} | Should not throw
    }
    It "Should Throw with array for ver "{
      { Start-ExeWithOutput -filepath "cmd.exe" @("/c", "ver", "/invalid") @Verbose } | Should throw
    }
  }
  ...
  It "Should not fail with array of args" {
    {
      Start-ExeWithOutput -filepath "whoami.exe" -args @("/ALL","/FO","LIST") @Verbose
    } | Should not throw
  }

    It "should throw on non 0 exit " {
      {
        Start-ExeWithOutput "whoami.exe","/invalid-arg"
      }| Should throw
    }

    It "Should not fail with array of args" {
    {
      Start-ExeWithOutput "nslookup.exe" @("-querytype=mx", "holliday.id.au") @Verbose
      } | Should not throw
    }

    It "Should not fail with array of args" {
      {Start-ExeWithOutput "nslookup.exe" @("-querytype=ns", "holliday.id.au") @Verbose} | Should not throw
    }
  ...
  It "Should Contain arguments in output" {
    Start-ExeWithOutput -filepath "echo-args.exe" @("arg1","arg2","arg3") | Should contain '1: arg1'
  }
